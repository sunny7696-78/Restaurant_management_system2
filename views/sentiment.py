...
def render_sentiment(rest_name: str):
    ...
    st.markdown(f"<small style='color:{P['muted']}'>Paste Google/Zomato/Swiggy reviews → Gemini analyses sentiment by category for <b>{rest_name}</b></small>", unsafe_allow_html=True)
    st.divider()
    ...
    with col_tip:
        ...
        <div style='background:#1A1A24;border:1px solid #2a2a3a;border-radius:10px;padding:16px'>
            <div style='font-size:12px;font-weight:700;color:{P["primary"]};margin-bottom:10px'>💡 How to get reviews</div>
            <div style='font-size:12px;color:{P["muted"]};line-height:1.8'>
                <b style='color:{P["text"]}'>Google Maps:</b><br>Search restaurant → Reviews tab → Copy<br><br>
                <b style='color:{P["text"]}'>Zomato:</b><br>Restaurant page → Reviews → Copy text<br><br>
                <b style='color:{P["text"]}'>Swiggy:</b><br>Restaurant → Ratings & Reviews → Copy
            </div>
    ...
    score_color = P["success"] if score >= 4 else (P["secondary"] if score >= 3 else P["danger"])
    ...
    h2.markdown(f"""<div class='kpi-card'>
        ...
        <div class='kpi-value' style='color:{P["success"]}'>{pos}%</div>
    </div>""", unsafe_allow_html=True)
    h3.markdown(f"""<div class='kpi-card'>
        <div class='kpi-label'>Neutral Reviews</div>
        <div class='kpi-value' style='color:{P["muted"]}'>{neu}%</div>
    </div>""", unsafe_allow_html=True)
    h4.markdown(f"""<div class='kpi-card'>
        <div class='kpi-label'>Negative Reviews</div>
        <div class='kpi-value' style='color:{P["danger"]}'>{neg}%</div>
    </div>""", unsafe_allow_html=True)

    # Summary
    if result.get("summary"):
        st.markdown(f"""
        <div style='background:#1A1A24;border:1px solid {P["primary"]}44;border-left:4px solid {P["primary"]};
                    border-radius:10px;padding:16px;margin:16px 0'>
            <div style='font-size:12px;color:{P["primary"]};font-weight:700;margin-bottom:6px'>📋 EXECUTIVE SUMMARY</div>
            <div style='font-size:14px;color:{P["text"]};line-height:1.7'>{result["summary"]}</div>
        </div>""", unsafe_allow_html=True)
    ...
    if cat_scores:
        ...
        with chart_col:
            ...
            colors_radar = [P["success"] if v >= 4 else (P["secondary"] if v >= 3 else P["danger"]) for v in vals]
            ...
            fig_radar = go.Figure(go.Scatterpolar(
                ...
                marker=dict(color=P["primary"], size=6),
            ))
            fig_radar.update_layout(
                polar=dict(
                    radialaxis=dict(visible=True, range=[0, 5], gridcolor="#2a2a38", color=P["muted"]),
                    angularaxis=dict(gridcolor="#2a2a38", color=P["text"]),
        ...
        with detail_col:
            for cat, val in cat_scores.items():
                c = P["success"] if val >= 4 else (P["secondary"] if val >= 3 else P["danger"])
                bar_w = int(val / 5 * 100)
                st.markdown(f"""
                <div style='margin-bottom:14px'>
                    <div style='display:flex;justify-content:space-between;margin-bottom:4px'>
                        <span style='font-size:13px;color:{P["text"]};font-weight:600'>{cat}</span>
    ...
    with pos_col:
        ...
        for pt in result.get("top_positives", []):
            st.markdown(f"""
            <div style='background:#0f2a26;border:1px solid {P["success"]}44;border-radius:8px;
                        padding:10px 14px;margin-bottom:8px;font-size:13px;color:{P["text"]}'>
                ✅ {pt}
            </div>""", unsafe_allow_html=True)
    ...
    with neg_col:
        ...
        for pt in result.get("top_negatives", []):
            st.markdown(f"""
            <div style='background:#2a0f12;border:1px solid {P["danger"]}44;border-radius:8px;
                        padding:10px 14px;margin-bottom:8px;font-size:13px;color:{P["text"]}'>
                ⚠️ {pt}
            </div>""", unsafe_allow_html=True)
    ...
    mcols    = st.columns(4)
    mcolors  = [P["primary"], P["secondary"], P["success"], P["accent"]]
    for i, cat in enumerate(CATEGORIES):
        ...
        <div class='kpi-card' style='text-align:left;border-color:{mcolors[i]}44'>
            ...
            <div style='font-size:12px;color:{P["text"]};line-height:1.6'>{insight}</div>
        </div>""", unsafe_allow_html=True)
    ...
    for i, action in enumerate(result.get("urgent_actions", []), 1):
        st.markdown(f"""
        <div style='background:#1A1A24;border:1px solid {P["secondary"]}44;border-left:4px solid {P["secondary"]};
                    border-radius:8px;padding:12px 16px;margin-bottom:8px;
                    display:flex;align-items:center;gap:12px'>
            <span style='font-size:16px;font-weight:700;color:{P["secondary"]};min-width:24px'>{i}.</span>
            <span style='font-size:13px;color:{P["text"]}'>{action}</span>
        </div>""", unsafe_allow_html=True)
    ...
    fig_pie = go.Figure(go.Pie(
        ...
        values=[pos, neu, neg],
        hole=0.5,
        marker=dict(colors=[P["success"], P["muted"], P["danger"]]),
